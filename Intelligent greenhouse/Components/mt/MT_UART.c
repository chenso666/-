...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  {     
	    //分配内存空间，为结构体内容+实际的数据（j个字节）+1个用于记录数据长度的字节
   pMsg = (mtOSALSerialData_t *)osal_msg_allocate( sizeof ( mtOSALSerialData_t )+j+1);  
         
  pMsg->hdr.event = CMD_SERIAL_MSG;  //8位事件CMD_SERIAL_MSG
  pMsg->msg = (uint8*)(pMsg+1);  // 把指针定位到结构体实际数据部分（1个用于记录数据长度的字节//+j个字节的数据）
  pMsg->msg [0]= j;     //1个用于记录数据长度的字节（j个字节）
  for(i=0;i<j;i++)    //从第二个元素开始记录实际数据（具体是哪j个字节） 
  pMsg->msg [i+1]= buf[i];   
